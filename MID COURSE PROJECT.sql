...
SELECT 
    ...
    MONTH(website_sessions.created_at) AS mth,
    COUNT(DISTINCT CASE
            WHEN website_sessions.utm_source = 'gsearch' THEN website_sessions.website_session_id
            ELSE NULL
        END) AS gsearch_sessions,
    COUNT(DISTINCT CASE
            WHEN website_sessions.utm_source = 'bsearch' THEN website_sessions.website_session_id
            ELSE NULL
        END) AS bsearch_sessions,
    COUNT(DISTINCT CASE
            WHEN
                website_sessions.utm_source IS NULL
...
SELECT 
    nonbrand_test_session_w_landing_pages.website_session_id,
    nonbrand_test_session_w_landing_pages.landing_page,
    orders.order_id AS order_id
FROM
    nonbrand_test_session_w_landing_pages
        LEFT JOIN
    orders ON orders.website_session_id = nonbrand_test_session_w_landing_pages.website_session_id;
...
-- 22973 sessions * 0.0087 incremental conversion = 201 incremental orders since 07/29
...
else 'uh oh .. check logic' end as segments,
count(distinct website_session_id) as sessions,
...
GROUP BY 1) as session_level_made_it
group by 1;
...
FROM
    ...
    WHERE
        website_pageviews.created_at > '2012-09-10'
            ...
            AND website_pageviews.pageview_url IN ('/billing' , '/billing-2')) AS billing_pageviews_and_orders
GROUP BY 1;
...
-- 1194 more sessions * 8.51 lift per billing session = 10160 last month